...
signed long int tmp;
volatile signed long int wdt_counter = 0;
signed long corrector1 = 0;
signed long corrector2 = 0;
unsigned char mode;
unsigned char P14OLD;
volatile unsigned char STROBE_EN;
...
//	P1SEL |= 0x01;                  			// P1.0 = ACLK
	...
	P1OUT &= ~0x01;
        ...
        P1REN &= ~0x08;                                          // P1.3 Pull up/down disable;
        //P1OUT |= 0x08;                                          // P1.3 Pulled up

        P1REN |= 0x10;                                          // P1.4 Pull up/down enable;
        P1OUT &= ~0x10;                                          // P1.4 Pulled down
...
//	tmp = MAX_PWM*0.5;
	...
	while(1)
	{
		if(P1IN & 0x08) //RED
		  ...
		  Threshold = 137;//200mv
//		  Threshold = 82;//120mv for tail white light
...
//		ADC_avg = ADC10MEM;
//	 	if((ADC_avg < Threshold) && (tmp < MAX_PWM+1)) tmp++;
//	 	if((ADC_avg > Threshold) && (tmp > 0)) tmp--;

	 	if((ADC_avg > Threshold+80))
                {
                  reg_counter -= 15;
                  //if(tmp > 0) tmp--;
                }
...
if(P1IN & 0x08)//RED
{
  if(wdt_counter == ((STROBE_PERIOD - STROBE_PULSE - STROBE_PULSE - STROBE_DELAY - 22)))
  {
    if(P1IN & 0x10)//sync line in HI state
      STROBE_EN = 1;
    else
      STROBE_EN = 0;
  }

   if(wdt_counter == ((STROBE_PERIOD - STROBE_PULSE - STROBE_PULSE - STROBE_DELAY - 20)))
     if(STROBE_EN)
       P1OUT |= 0x80;

   if((STROBE_EN) && (wdt_counter == STROBE_PERIOD - STROBE_PULSE - STROBE_PULSE - STROBE_DELAY))
   {
	 P1OUT |= 0x01;
         P1OUT &= ~0x80;
   }
     ...
     if(wdt_counter == STROBE_PERIOD - STROBE_PULSE - STROBE_DELAY)
       ...
       if((STROBE_EN) && (wdt_counter == STROBE_PERIOD - STROBE_PULSE))
	     P1OUT |= 0x01;
...
{
  if( P14OLD != (P1IN & 0x10) )//Sync change
  {
    if(!(P1IN & 0x10))//Low
    {
      if(STROBE_EN == 0)
    ...
    {
      if((wdt_counter > 15) && (wdt_counter < 25))
      {
        wdt_counter = STROBE_PERIOD - STROBE_PULSE - STROBE_PULSE - STROBE_DELAY;
        STROBE_EN = 1;
      }
    }
  }
