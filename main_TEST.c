...
#define STROBE_DELAY    20//40ms
#define	STROBE_PERIOD	700//1400ms 1800ms
...
unsigned long corrector1 = 0;
unsigned long corrector2 = 0;
unsigned char mode;
unsigned char P14OLD;
unsigned int StrobePulse = 1;
...
//  ADC_avg = (ADC10MEM + ADC_avg * 7)>>3;
  ADC_avg = (ADC10MEM + ADC_avg)>>1;
                  tmp = ADC_avg / 7;

                if(tmp < 1) StrobePulse = 1;//2ms
                  else
                if(tmp > 100) StrobePulse = 100;//200ms
                 else
                  StrobePulse = tmp;
  
{
   ...
   if(wdt_counter == ((STROBE_PERIOD - StrobePulse - StrobePulse - StrobePulse)/2))
     P1OUT |= 0x80;
...
//if(P1IN & 0x08)   
    
   {
     if( (P14OLD != (P1IN & 0x10)) && (!(P1IN & 0x10)) )
         wdt_counter = ((STROBE_PERIOD - StrobePulse - StrobePulse - StrobePulse)/2)+1;
     P14OLD = (P1IN & 0x10);
   }   

   if(wdt_counter == STROBE_PERIOD - StrobePulse - StrobePulse - StrobePulse)
	 P1OUT |= 0x01;
   else
     if(wdt_counter == STROBE_PERIOD - StrobePulse - StrobePulse)
	   P1OUT &= ~0x01;
     else     
       if(wdt_counter == STROBE_PERIOD - StrobePulse)
	     P1OUT |= 0x01;
       else     
